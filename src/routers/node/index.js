...
router.post('nodes',
	...
	async function(ctx, next) {
		const { name, host, port, username, protocol, state = true } = ctx.request.body;
		...
		const node = await Node.create({ name, host, port, username, protocol, state }).catch(error => ctx.customResponse.error(error.message));
